...
var commands = exports.commands = {
	...
	rbydex: 'rbydata',
	rbypokedex: 'rbydata',
	rbydata: function(target, room, user) {
		if (!this.canBroadcast()) return;
		...
		function getTier(poke) {
			var FormatsData = poke.formatsData;
		...
		var data = '';
		var targetId = toId(target);
		var newTargets = Tools.mod('gen1').dataSearch(target);
		if (newTargets && newTargets.length) {
			for (var i = 0; i < newTargets.length; i++) {
				if (newTargets[i].id !== targetId && !Tools.data.Aliases[targetId] && !i) {
					data = "No Pokemon, item, move or ability named '" + target + "' was found. Showing the data of '" + newTargets[0].name + "' instead.\n";
				...
				if (newTargets[i].searchType === 'pokemon' && !Tools.data.Pokedex[newTargets[i].id]) {
					data += '|c|~OiaBot|'+ '' + newTargets[i].name + ' - ' + newTargets[i].types.join('/') + ' | Stats:' + getStats(newTargets[i]) + ' | ' + getBST(newTargets[i]) + ' BST';
				} else {
					data += '|c|~OiaBot|'+ '' + newTargets[i].name + ' - ' + newTargets[i].types.join('/') + ' | Stats:' + getStats(newTargets[i]) + ' | ' + getBST(newTargets[i]) + ' BST';
				}
			}
	...
	suspect: function(target, room, user) {
		...
		if (!target || target === 'all') {
			matched = true;
			buffer += '|c|~OiaBot|Suspects and stuff';
		}
		if (target === 'gen4ou') {
			matched = true;
			buffer += '|c|~OiaBot|Our current Gen 4 (DPPt) OU Suspects are the **retesting of Latias and Salamence from the gen 4 overused tier**';
		}
		if (target === 'gen4nu') {
			matched = true;
			buffer += '|c|~OiaBot|Our current Gen 4 (DPPt) NU Suspects are the **banning of Espeon and Entei from the gen 4 Neverused tier**';
		}
		if (target === 'gen2uu') {
			matched = true;
			buffer += '|c|~OiaBot|Our current Gen 2 (GSC) UU Suspects are the **Moving of Venusaur and Smeargle into BL-Borderline tier- from the gen 2 Underused tier**';
		}
	...
	emote: function(target, room, user) {
		if (!this.canBroadcast()) return;
		this.sendReply('|raw| <img src="https://s3.amazonaws.com/mibbit/gfx/smilies/yel3'+target+'.png">');
	},
...
this.sendReply("User " + targetUser.name + " is no longer using that name.");
	...
	oiabot: function(target, room, user) {
		if (!this.canBroadcast()) return;
		target = toId(target);
		var buffer = '';
		var matched = false;
		if (target === 'about') {
			matched = true;
			buffer += '|c|~OiaBot|OiaBot is a bot created by user Oiawesome for the purpose displaying random data and data of Old Gen pokemon due to having a bot being hilariously failtastic';
		}
		if (target === 'quotecool') {
			matched = true;
			buffer += '|c|~OiaBot|"[14:31:13] ~Oiawesome: man Oiabot is so cool" MAN I FUCKING LOVE THIS GUY';
		}
		if (target === 'quoteinspiration') {
			matched = true;
			buffer += '|c|~OiaBot|"[15:01]&V4: if cant find - [15:01] &V4: make a way"- greatest quote, so inspirational .cry';
		}
		if (!matched) {
			buffer += '|c|~OiaBot|The OiaBot entry "'+target+'" was not found.';
		}
		this.sendReply(buffer);
	},
	...
	wiki: function(target, room, user) {
		if (!this.canBroadcast()) return;
		this.sendReply('|c|~OiaBot|http://en.wikipedia.org/wiki/'+target+'');
	},

	define: function(target, room, user) {
		if (!this.canBroadcast()) return;
		this.sendReply('|c|~OiaBot|https://www.google.com/search?q=define+amazing&oq=define+'+target+'');
	},
